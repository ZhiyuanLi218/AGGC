...
class FSDPActorConfig(ActorConfig):
    ...
    use_rollout_log_probs: bool = False
    # LoRA 分模块梯度裁剪配置（可选）
    per_module_grad_norm: "PerModuleGradNormConfig" = field(default_factory=lambda: PerModuleGradNormConfig())
...
@dataclass
class PerModuleGradNormConfig(BaseConfig):
    """LoRA 分模块梯度范数裁剪配置。

    与 utils.lora_grad_clip.PerModuleGradNormOptions 字段对应。
    """

    enable: bool = False
    apply_to: str = "lora_only"  # 'lora_only' | 'all_trainable'
